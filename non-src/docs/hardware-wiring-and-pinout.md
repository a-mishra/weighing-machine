...
- `LED_RED_PIN` -> GPIO `6` (Pico physical pin 9)
- `LED_YELLOW_PIN` -> GPIO `7` (Pico physical pin 10)
- `LED_GREEN_PIN` -> GPIO `8` (Pico physical pin 11)

Use a current-limiting resistor per LED (typ. 220-330 ohm). Common cathode: GPIO high = on.

## 2.5 Optional Buzzer (active 5V)

- `BUZZER_PIN` -> GPIO `15` (Pico physical pin 20)

Wiring:
- Buzzer `+` -> 5V (not Pico 3.3V for 5V modules)
- Buzzer `-` -> NPN transistor collector; emitter -> GND
- Base (via ~1k) -> GPIO15
- Common GND with Pico

GPIO high turns the buzzer on (`BUZZER_ACTIVE_HIGH = True`). Invert in `config.py` if your module is low-trigger.
...
| Encoder | + | 3.3V |
| Encoder | GND | GND |
| Status LED red | Anode (via resistor) | GPIO6 (pin 9) |
| Status LED yellow | Anode (via resistor) | GPIO7 (pin 10) |
| Status LED green | Anode (via resistor) | GPIO8 (pin 11) |
| Buzzer (optional) | SIG | GPIO15 (pin 20) |
| Buzzer (optional) | VCC | 5V (via driver) |
